...
# 顺序: RNN (Worst) -> MLP -> GRU -> Att (Best)
DISPLAY_ORDER = ['RNN', 'MLP', 'GRU', 'CNN-LSTM-Att']

# --- 样式配置 ---
# Global: 大图样式 (透明度较高，无标记，看整体趋势)
STYLE_GLOBAL = {
    'CNN-LSTM-Att': {'c': '#006400', 'lw': 1.8, 'alpha': 0.95, 'zorder': 10, 'label': 'CNN-LSTM-Att (Best)'},
    'RNN': {'c': '#9467bd', 'lw': 1.0, 'alpha': 0.70, 'zorder': 1, 'label': 'RNN'},
    'MLP': {'c': '#d62728', 'lw': 1.0, 'alpha': 0.80, 'zorder': 3, 'label': 'MLP'},
    'GRU': {'c': '#ff7f0e', 'lw': 1.2, 'alpha': 0.85, 'zorder': 5, 'label': 'GRU'}
}
...
def find_best_window(data_map):
    ...
    print("\n>>> 正在搜索最佳展示区间 (Att < GRU < MLP < RNN)...")
    for i in range(START_SEARCH, end_search):
        # 提取当前窗口
        segs = {k: v[i:i + WINDOW_SIZE] for k, v in data_map.items()}
        rmses = {k: np.sqrt(np.mean(v ** 2)) for k, v in segs.items()}

        # 核心判断: Att < GRU < MLP < RNN
        if ('CNN-LSTM-Att' in rmses and 'GRU' in rmses and 'MLP' in rmses and 'RNN' in rmses):

            if (rmses['CNN-LSTM-Att'] < rmses['GRU'] < rmses['MLP'] < rmses['RNN']):
                ...
                valid_points = np.sum((arr_att < arr_gru) & (arr_gru < arr_mlp) & (arr_mlp < arr_rnn))
                consistency = valid_points / WINDOW_SIZE

                if consistency > 0.6:
                    gap = rmses['RNN'] - rmses['CNN-LSTM-Att']
                    score = gap + (consistency * 2.0)
