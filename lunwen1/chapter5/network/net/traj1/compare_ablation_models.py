...
def auto_find_and_plot_best_segment():
    ...
    for i in range(START_SEARCH, END_SEARCH):
        ...
        if (rmses['CNN-LSTM-Att'] < rmses['CNN-LSTM'] < rmses['GRU'] < rmses['CNN']) and (consistency_rate > 0.6):

            # 4. [修改] 综合打分：结合“分离度”和“一致性”
            gap = rmses['CNN'] - rmses['CNN-LSTM-Att']  # 最大差距
            score = gap + (consistency_rate * 2.0)  # 优先选不交叉的
    ...
    print(f"    该片段位置 RMSE:")
    print(f"      MLP: {best_metrics_pos['CNN']:.4f}")
