...
FEATURE_MODE = 'POS'  # <--- 每次训练前改这里！

if FEATURE_MODE == 'POS':
    INPUT_DIM = 3
    MODEL_FILE = 'data_n/pos/imm_param_net_pos.pth'
    SCALER_FILE = 'data_n/pos/scaler_params_pos.json'
elif FEATURE_MODE == 'POS_VEL':
    INPUT_DIM = 6
    MODEL_FILE = 'data_n/pos_vel/imm_param_net_pos_vel.pth'
    SCALER_FILE = 'data_n/pos_vel/scaler_params_pos_vel.json'
else:
    INPUT_DIM = 9
    MODEL_FILE = 'data_n/pos_vel_acc/imm_param_net.pth'
    SCALER_FILE = 'data_n/pos_vel_acc/scaler_params.json'
...
def main():
    ...
    try:
        X_raw, Y_raw, G_raw = load_and_merge_data(os.path.join('../../npz', 'training_data_part*.npz'))
        if FEATURE_MODE == 'POS':
            print(f">>> [Ablation] 仅保留位置特征 (First 3 dims)")
            X_raw = X_raw[:, :, 0:3]
        elif FEATURE_MODE == 'POS_VEL':
            print(f">>> [Ablation] 保留位置+速度特征 (First 6 dims)")
            X_raw = X_raw[:, :, 0:6]
        else:
            print(f">>> [Ablation] 使用全量特征 (All 9 dims)")
    ...
    # 6. 模型初始化
    model = ParamPredictorMLP(seq_len=90, input_dim=INPUT_DIM).to(device)
