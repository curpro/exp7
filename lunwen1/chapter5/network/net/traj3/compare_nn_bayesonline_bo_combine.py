...
WINDOW_SIZE = 100
START_SEARCH = 80  # 与原代码保持一致
MARK_EVERY = 10  # 标记间隔
MANUAL_OFFSET = -25  # 保留原文件中的手动偏移
...
def find_best_window(data_map):
    ...
    for i in range(START_SEARCH, end_search):
        ...
        # 2. 一致性 (Consistency): 绿线在红线下方的比例
        is_lower = seg_nn < seg_bayes
        consistency = np.mean(is_lower)

        # 3. 间距 (Gap): 红色减去绿色，越大越好
        gap = np.mean(seg_bayes - seg_nn)

        # 4. 惩罚 (Penalty): 也就是绿线偶尔跑道红线上面去的幅度，要重罚
        violation = np.sum(np.maximum(0, seg_nn - seg_bayes))

        # 5. 综合评分
        score = (consistency * w_consistency) + (gap * w_gap) - (violation * w_penalty)
