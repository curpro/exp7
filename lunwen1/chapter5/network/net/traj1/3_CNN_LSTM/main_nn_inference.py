...
class ParamPredictorEnhanced(nn.Module):
    ...
    def forward(self, x):
        ...
        # --- CNN Block ---
        # 调整维度适配 Conv1d: (Batch, 9, 90)
        x = x.transpose(1, 2)

        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        # CNN Output: (Batch, 256, 90)

        # --- LSTM Block ---
        # 调整维度适配 LSTM: (Batch, 90, 256)
        x = x.transpose(1, 2)

        # self.lstm 输出: (Batch, Seq_Len, Num_Directions * Hidden_Size)
        ...
        # Log Softmax 保证每行概率和为1 (配合 KLDivLoss)
...
def main_inference():
    ...
    # 1. 加载模型与参数
    try:
        model = ParamPredictorEnhanced(input_dim=9, output_dim=9).to(device)
    ...
    save_filename = f'nn_results_CNN_LSTM.npz'
