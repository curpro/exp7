...
def find_best_window(data_map):
    ...
    print("\n>>> 正在搜索最佳展示区间 (Att < LSTM < GRU < CNN)...")
    for i in range(START_SEARCH, end_search):
        # 提取当前窗口
        segs = {k: v[i:i + WINDOW_SIZE] for k, v in data_map.items()}
        rmses = {k: np.sqrt(np.mean(v ** 2)) for k, v in segs.items()}

        # 核心判断: Att < LSTM < GRU < CNN
        # 注意: 原文的 GRU 其实对应的是 label 'LSTM', 变量名是 'GRU'
        if ('CNN-LSTM-Att' in rmses and 'CNN-LSTM' in rmses and 'GRU' in rmses and 'CNN' in rmses):

            if (rmses['CNN-LSTM-Att'] < rmses['CNN-LSTM'] < rmses['GRU'] < rmses['CNN']):

                # 计算一致性 (不交叉的点数比例)
                arr_att = segs['CNN-LSTM-Att']
                arr_lstm = segs['CNN-LSTM']
                arr_gru = segs['GRU']
                arr_cnn = segs['CNN']

                valid_points = np.sum((arr_att < arr_lstm) & (arr_lstm < arr_gru) & (arr_gru < arr_cnn))
                consistency = valid_points / WINDOW_SIZE

                # 只有一致性较高才考虑
                if consistency > 0.6:
                    gap = rmses['CNN'] - rmses['CNN-LSTM-Att']
                    # 综合打分：间距 + 一致性权重
