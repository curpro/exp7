...
def find_best_window(data_map):
    ...
    (复刻 compare_ablation_models.py 的离散度打分逻辑)
    """
    ...
    print("\n>>> 正在搜索最佳展示区间 (Score=StdDev)...")
    # 原代码步长为 2
    for i in range(START_SEARCH, end_search, 2):
        # 1. 提取当前窗口数据
        d1 = data_map['CNN-LSTM-Att'][i: i + WINDOW_SIZE]
        d2 = data_map['CNN-LSTM'][i: i + WINDOW_SIZE]
        d3 = data_map['GRU'][i: i + WINDOW_SIZE]
        d4 = data_map['CNN'][i: i + WINDOW_SIZE]

        # 2. 基础门槛：Att 总体误差要是最小的
        if not (np.mean(d1) < np.mean(d2)):
            continue

        # 3. 计算“离散度” (Standard Deviation)
        # 把4个模型堆叠起来，计算每一个时间点上，这4个数值的标准差
        stack = np.vstack([d1, d2, d3, d4])
        std_devs = np.std(stack, axis=0)

        # 4. 核心得分：总离散度 (Sum of StdDev)
        # 分数越高，说明这4条线分得越开
