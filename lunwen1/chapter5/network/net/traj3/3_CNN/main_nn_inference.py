...
class PureCNN(nn.Module):
    def __init__(self, input_dim=9, output_dim=9):
        ...
        # [关键] 我们不继续池化了，保留这 22 个时间步的信息
        # 维度计算: 64通道 * 22长度 = 1408
        self.cnn_flat_dim = 64 * 22

        # 全连接层
        self.fc = nn.Sequential(
            nn.Linear(self.cnn_flat_dim, 128),  # 接收所有时间步的信息
            nn.BatchNorm1d(128),
            nn.LeakyReLU(0.1),
            nn.Dropout(0.4),
    ...
    def forward(self, x):
        # x: (Batch, 90, 9)
        x = x.transpose(1, 2)  # -> (Batch, 9, 90)

        x = self.cnn(x)  # -> (Batch, 64, 22)

        # [核心] 展平，保留空间/时间结构
        x = x.flatten(1)  # -> (Batch, 1408)

        logits = self.fc(x)
        logits = logits.view(-1, 3, 3)
        return torch.log_softmax(logits / 2.0, dim=2)
...
def main_inference():
    ...
    meas_noise_std = 15.5  # 对齐 BoIMM
    noise_matrix = np.random.randn(3, sim_steps) * meas_noise_std
    _ = np.random.randn(9)

    # 1. 加载模型与参数
    try:
        model = PureCNN(input_dim=9,output_dim=9).to(device)
    ...
    alpha_smooth = 0.9 #todo 建议调小一下 待定
    ...
    for k in range(sim_steps):
        ...
        if len(pos_buffer) == WINDOW_SIZE and k % OPTIMIZE_INTERVAL == 0:
            ...
            cnn_inference_times.append(t_cost)
    ...
    avg_time = np.mean(cnn_inference_times)
    std_time = np.std(cnn_inference_times)
    ...
    np.save('cnn_inference_times.npy', np.array(cnn_inference_times))
    print(">>> 已保存: cnn_inference_times.npy")


    save_filename = f'nn_results_CNN.npz'
