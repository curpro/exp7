...
FEATURE_MODE = 'POS'  # <--- 必须与训练时的模式一致！

if FEATURE_MODE == 'POS':
    INPUT_DIM = 3
    MODEL_PATH = 'data_n/pos/imm_param_net_pos.pth'
    SCALER_PATH = 'data_n/pos/scaler_params_pos.json'
elif FEATURE_MODE == 'POS_VEL':
    INPUT_DIM = 6
    MODEL_PATH = 'data_n/pos_vel/imm_param_net_pos_vel.pth'
    SCALER_PATH = 'data_n/pos_vel/scaler_params_pos_vel.json'
elif FEATURE_MODE == 'POS_VEL_ACC':
    INPUT_DIM = 9
    MODEL_PATH = 'data_n/pos_vel_acc/imm_param_net.pth'
    SCALER_PATH = 'data_n/pos_vel_acc/scaler_params.json'
...
def main_inference():
    ...
    # 1. 加载模型与参数
    try:
        model = ParamPredictorMLP(seq_len=WINDOW_SIZE, input_dim=INPUT_DIM).to(device)
    ...
    for k in range(sim_steps):
        ...
        if len(pos_buffer) == WINDOW_SIZE and k % OPTIMIZE_INTERVAL == 0:
            ...
            if FEATURE_MODE == 'POS':
                raw_features = rel_pos_seq  # 只有位置 (N, 3)
            elif FEATURE_MODE == 'POS_VEL':
                raw_features = np.hstack([rel_pos_seq, vel_seq])  # 位置+速度 (N, 6)
            else:
                raw_features = np.hstack([rel_pos_seq, vel_seq, acc_seq])  # 全量
    ...
    save_filename = f'nn_results_{FEATURE_MODE}.npz'
