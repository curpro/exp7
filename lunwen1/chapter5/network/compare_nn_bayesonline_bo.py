...
def auto_find_and_plot_best_segment():
    ...
    for i in range(START_SEARCH, END_SEARCH):
        ...
        seg_bo = data_map_pos['Bo-IMM'][i: i + WINDOW_SIZE]

        # 1. 计算均方根误差 (仅用于最终显示，不完全用于排序)
        rmses = {
            'NN-IMM': np.sqrt(np.mean(seg_nn ** 2)),
            'bayesOnline': np.sqrt(np.mean(seg_bayes ** 2)),
            'Bo-IMM': np.sqrt(np.mean(seg_bo ** 2))
        }

        # 2. 计算点对点的一致性 (Consistency)
        # 我们希望: nn < bayes 且 bayes < bo
        # mask 是一个布尔数组，True 表示该帧满足顺序
        mask = (seg_nn < seg_bayes) & (seg_bayes < seg_bo)

        # 满足条件的帧数比例 (0.0 ~ 1.0)，我们希望这个越接近 1.0 越好
        consistency_rate = np.mean(mask)

        # 3. 计算分离度 (Gap)
        # 只有当整体均值也满足顺序时，计算 Gap 才有意义，否则 Gap 设为 0
        if rmses['NN-IMM'] < rmses['bayesOnline'] < rmses['Bo-IMM']:
            gap1 = rmses['bayesOnline'] - rmses['NN-IMM']
            gap2 = rmses['Bo-IMM'] - rmses['bayesOnline']
