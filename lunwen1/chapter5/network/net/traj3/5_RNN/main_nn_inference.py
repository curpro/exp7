...
# ================= 配置 =================
# [修改] 指向您的 F16 测试文件路径
TEST_DATA_PATH = r'D:\AFS\lunwen\dataSet\processed_data_4\f16_complex_data_razor.csv'
...
class ParamPredictorRNN(nn.Module):
    def __init__(self, n_inputs=9, n_outputs=9,
                 hidden_size=128, num_layers=2, dropout=0.2):
        super().__init__()
        assert int(np.sqrt(n_outputs)) ** 2 == n_outputs
        self.n_models = int(np.sqrt(n_outputs))
        ...
        feat_dim = hidden_size * 2  # 双向
        # mean + max 拼接 -> 2 * feat_dim
        self.ln = nn.LayerNorm(feat_dim * 2)
    ...
    def forward(self, x):
        ...
        # ✅ 无 attention 的强聚合：mean + max pooling
        mean_pool = out.mean(dim=1)                 # (B, 2H)
        max_pool = out.max(dim=1).values            # (B, 2H)
        feat = torch.cat([mean_pool, max_pool], dim=1)  # (B, 4H)

        feat = self.ln(feat)
        logits = self.head(feat)                    # (B, 9)
        logits = logits.view(-1, self.n_models, self.n_models)  # (B,3,3)
...
def main_inference():
    ...
    # 1. 加载模型与参数
    try:
        model = ParamPredictorRNN(n_inputs=9, n_outputs=9).to(device)
    ...
    save_filename = f'nn_results_RNN.npz'
    ...
    print("绘制计算开销对比图...")
    # 1. 加载 BO 的实测数据
    if os.path.exists('../../bayes_imm/result/bo_costs_measured.npy'):
        real_bo_times = np.load('../../bayes_imm/result/bo_costs_measured.npy')
    ...
    end_f = min(sim_steps, max_err_idx)
